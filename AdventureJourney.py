...
User can select from different characters(e.g. a wizard, dwarf, elf, etc)
Each of these characters have differences in strength, defence, magic, health and gold
Provide a number of scenarios where they meet potential friends or enemies and have choices to make 
Friends, if user makes the right choice, will give characters money
There are different enemies that may appear based on user choice
These enemies have different strength, defences and health
User and enemy battle
If user gets to zero health, the game ends
BONUS: Sometimes there's an element of actual chance if an enemy will appear or not
BONUS: When user creates their character, they also are given 5 points that they can add to their strength, defence, etc
BONUS: Put unecessarily repeated code into a function '''
import time
import random
def print_pause(message_to_print):
    ...
    time.sleep(1.8)
def intro():
    print_pause("You are born in an era where the evil demon 'Xi Gong Di' reigns")
    print_pause("Fed up by all the injustice, you seek to find the only weapon to defeat the demon")
    print_pause("The weapon forged from the powers of Light and Dark")
    print_pause("You embark on your journey with only a piece of bread and the clothes on your back")
    print_pause("You go out of your house and see the familiar slum that you grew up in")
...
def fight_intro(items):
    ...
    print_pause("The demon 'Xi Ging Di' sees you!")
    print_pause("'Xi Gong Di' lunges at you! His sharp rotten teeth bared to bite!")
...
def capital_intro(items):
    ...
    print_pause("Tall structures rise, but behind this facade of progress and luxury, evil and corruption exists.")
    if 'bread' in items:
        print_pause("At the corner, you caught the gaze of an old man")
        print_pause("The old man hesitates at first but approaches you anyway")
        encounter_man(items)
    else:
...
def encounter_man(items):
    ...
    if action=="1":
        print_pause("'Can you spare me some food?' the old man said")
        print_pause("You only have a piece of bread with you and nothing else")
        food_choice(items)
    elif action=="2":
        ignore_man(items)
    else:
...
def receive_weapon(items):
    ...
    print_pause("'Despite not having enough for yourself, you still gave your food to me'")
    print_pause("'Truly, you have a good heart. You may be our only hope.'")
    print_pause("'Please take this and defeat the demon that has corrupted our land.'")
    print_pause("You have gained the legendary weapon ! The Chaos Breaker!")
    items.append("Chaos Breaker")
    main_choice(items)
def ignore_man(items):
    print_pause("You averted your gaze from the old man and continued wandering")
